#!/usr/bin/env python3
"""
Dump GPOS lookups from TTF font to CSV file.
Extracts all GPOS lookup information including type, flags, and subtable count.
"""
import argparse
import csv
import sys
...
def extract_gpos_lookups(font):
    """
    Extract GPOS lookup information from font.
...
def dump_gpos_lookups_csv(input_ttf, output_csv, font_index=0):
    """
    Extract GPOS lookups and write to CSV file.
    ...
    lookups = extract_gpos_lookups(font)
    font.close()
    
    if not lookups:
        print(f"No GPOS lookups found in {input_ttf}", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"GPOS lookups written to {output_csv}")
    print(f"Total lookups: {len(lookups)}")


def main():
    parser = argparse.ArgumentParser(
        description='Dump GPOS lookups from TTF/TTC font to CSV file.'
    )
    parser.add_argument(
        'input_ttf',
    ...
    dump_gpos_lookups_csv(args.input_ttf, args.output_csv, args.font_index)
