#!/usr/bin/env python3
"""
Dump GSUB features from TTF font to CSV file.
Extracts all GSUB feature information including tags, indices, and associated scripts.
"""
import argparse
import csv
import sys
...
def extract_gsub_features(font):
    """
    Extract GSUB feature information from font.
...
def dump_gsub_features_csv(input_ttf, output_csv, font_index=0):
    """
    Extract GSUB features and write to CSV file.
    ...
    features = extract_gsub_features(font)
    font.close()
    
    if not features:
        print(f"No GSUB features found in {input_ttf}", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"GSUB features written to {output_csv}")
    print(f"Total features: {len(features)}")


def main():
    parser = argparse.ArgumentParser(
        description='Dump GSUB features from TTF/TTC font to CSV file.'
    )
    parser.add_argument(
        'input_ttf',
    ...
    dump_gsub_features_csv(args.input_ttf, args.output_csv, args.font_index)
